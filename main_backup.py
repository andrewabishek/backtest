    ...
    for i in tqdm(range(len(df)), desc="Processing bars", unit="bars"):
        ...
        if (pivot_high and pivot_high_time not in used_pivots and 
            ...
            if trade_info:
                if trade_info['entered']:
                    # Trade entered immediately
                    trades.append(trade_info['trade'])
                    used_pivots.add(pivot_high_time)
                    logger.info(f"Trade #{len(trades)} entered immediately: Long @ {trade_info['trade']['entry_price']:.1f}")
                else:
        ...
        if (pivot_low and pivot_low_time not in used_pivots and 
            ...
            if trade_info:
                if trade_info['entered']:
                    ...
                    logger.info(f"Trade #{len(trades)} entered immediately: Short @ {trade_info['trade']['entry_price']:.1f}")
                else:
                    # Trade setup pending entry
        ...
        for trade_info in open_trades:
            if not trade_info['entered'] and check_entry_trigger(row, trade_info):
                ...
                entered_trades.append(trade_info)
...
def close_trade_at_market_close(df, close_idx, trade_info):
    ...
    # Entry logic: wait for break of marking candle level within 18 bars after breakout
    entry_idx = None
    max_bars = 18
    updates = 0
    last_sl = initial_sl_price
    last_entry = initial_entry_price
    
    # Track marking candle updates
    for k in range(marking_idx + 1, min(breakout_idx + max_bars + 1, len(df))):
        if k >= len(df):
            break
        ...
        if updates < 3:
            if direction == 'long' and row['close'] < row['open']:
                if row['low'] < last_sl - 1:  # SL extension check
                    new_sl_distance = abs(last_entry - row['low'])
                    if new_sl_distance <= 25:  # Check 25-point limit
                        last_sl = row['low']
                        last_entry = row['high']
                        updates += 1
            elif direction == 'short' and row['close'] > row['open']:
                if row['high'] > last_sl + 1:  # SL extension check
                    new_sl_distance = abs(row['high'] - last_entry)
                    if new_sl_distance <= 25:  # Check 25-point limit
                        last_sl = row['high']
                        last_entry = row['low']
                        updates += 1
        ...
        # Entry trigger
        if direction == 'long' and row['high'] > last_entry:
            entry_idx = k
            break
        elif direction == 'short' and row['low'] < last_entry:
            entry_idx = k
            break
    ...
    rr = 2  # Risk-reward ratio
    tp_price = entry_price + rr * sl_distance if direction == 'long' else entry_price - rr * sl_distance
    
    # Track max favorable and adverse excursion
    max_favorable = 0
    max_adverse = 0
    
    # Trade management: exit at SL or TP
    for m in range(entry_idx + 1, len(df)):
        row = df.iloc[m]
        ...
        max_favorable = max(max_favorable, current_pl)
        max_adverse = max(max_adverse, current_adverse)
        
        # Exit conditions
        if direction == 'long':
            if row['low'] <= sl_price:
                return create_trade_result(df, breakout_idx, entry_idx, m, direction, 
                                         entry_price, sl_price, tp_price, sl_price, 'SL',
                                         max_favorable, max_adverse, pivot_level, updates)
            if row['high'] >= tp_price:
                return create_trade_result(df, breakout_idx, entry_idx, m, direction,
                                         entry_price, sl_price, tp_price, tp_price, 'TP',
                                         max_favorable, max_adverse, pivot_level, updates)
        else:
            if row['high'] >= sl_price:
                return create_trade_result(df, breakout_idx, entry_idx, m, direction,
...
def create_trade_result(df, breakout_idx, entry_idx, exit_idx, direction, 
                       entry_price, sl_price, tp_price, exit_price, result,
                       max_favorable, max_adverse, pivot_level, updates):
    ...
    return {
        'trade_num': None,  # Will be set later
        'breakout_time': df.iloc[breakout_idx]['datetime'],
        'entry_time': entry_time,
        'exit_time': exit_time,
        'direction': direction,
        'pivot_level': pivot_level,
        ...
        'max_rr_potential': max_favorable / sl_distance if sl_distance > 0 else 0,
        'time_in_trade_minutes': time_in_trade.total_seconds() / 60,
        'marking_updates': updates
    }
